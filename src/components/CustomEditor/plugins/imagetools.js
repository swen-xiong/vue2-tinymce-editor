 ...
 * 从 tinymce 5.10.9 中复制而来
 * 原因：tinymce v5.10 之后的版本将该插件从免费版中移除，加入到了收费版本
 *
 * 由于本地使用的是 v6.2.0 版本，所以需要修改部分代码
 * 原插件中有 `api.setDisabled(!isEditableImage)` ，修改为 `api.setEnabled(isEditableImage)`
 *
 * 当图片通过拖拽插入时，选中或取消选中图片控制台会报错：`Uncaught TypeError: global$3.clearTimeout is not a function`
 * 这是由于没有实现 images_upload_handler 导致，实现图片上传功能后，该问题解决
 */
...
;(function () {
        ...
        return {
          fold: function () {
            ...
            if (foldArgs.length !== cases.length) {
              throw new Error(
                'Wrong number of arguments to fold. Expected ' + cases.length + ', got ' + foldArgs.length
              )
            }
  ...
  var isServiceErrorCode = function (code, blob) {
    return (
      (blob === null || blob === void 0 ? void 0 : blob.type) === 'application/json' &&
      (code === 400 || code === 403 || code === 404 || code === 500)
    )
  }
  ...
  var requestServiceBlob = function (url, apiKey) {
    ...
    return sendRequest(appendApiKey(url, apiKey), headers).then(function (result) {
      return isError(result.status)
        ? handleServiceErrorResponse(result.status, result.blob)
        : Promise$1.resolve(result.blob)
    })
  }
  ...
  var getEditableImage = function (editor, node) {
    var isEditable = function (imgNode) {
      return (
        isImage(editor, imgNode) &&
        (isLocalImage(editor, imgNode) || isCorsImage(editor, imgNode) || isNonNullable(getProxyUrl(editor)))
      )
    }
  ...
  var isLocalImage = function (editor, img) {
    var url = img.src
    return (
      url.indexOf('data:') === 0 || url.indexOf('blob:') === 0 || new global$1(url).host === editor.documentBaseURI.host
    )
  }
  ...
  var startTimedUpload = function (editor, imageUploadTimerState) {
    var imageUploadTimer = global$3.setEditorTimeout(
      editor,
      function () {
  ...
  var setup = function (editor, imageUploadTimerState, lastSelectedImageState) {
    editor.on('NodeChange', function (e) {
      ...
      if (
        lastSelectedImage &&
        !selectedImage.exists(function (img) {
  ...
  var register$1 = function (editor) {
    ...
    var onSetup = function (api) {
      var handler = function (isEditableImage) {
        return api.setEnabled(isEditableImage)
      }
